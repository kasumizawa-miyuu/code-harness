...
|------|------|
| 输入 | 用户上传的 zip 压缩包 |
| 行为 | 自动检测云端环境 → 提示用户上传工作区 → 解压到临时目录 → 设置 workDir → 允许 Agent 操作 |
| 输出 | 工作区文件树、可下载的修改后 zip |
| 边界条件 | 未上传工作区时拒绝运行任务；上传 zip 过大（>100MB）拒绝；路径穿越攻击拒绝 |
| 错误处理 | 解压失败 → 提示文件损坏；临时目录创建失败 → 提示系统错误 |
